...
For every finding, explain the failure scenario, affected users or code path, and a concrete fix. Require regression coverage for the scenario, using `androidTest` or a Maestro flow for critical user journeys when JVM tests cannot exercise it. Treat missing tests as a finding only when the change has a meaningful untested behavior; avoid style-only comments. End with a short risk summary and list the validation commands actually run.
